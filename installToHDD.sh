...
printListOfKeymaps(){
	find "${1}/usr/"*"-linux-gnu/etc/console-setup/ckb/symbols/" -maxdepth 1 -type f | sort | while read line4; do
		basename "${line4}"
	done
}
...
while true; do
	...
	if [ "${option4}" = "Y" ] || [ "${option4}" = "y" ]; then
		...
		echo ""
		dd bs=440 count=1 conv=notrunc if="$(find "/usr/"*"/share/syslinux" -name mbr.bin | head -n 1)" of=${1}
		echo ""
		break
